...
| G 轮 | AI 相关内容 | 状态 |
| :-- | :-- | :-- |
| G0 | 范式重排（移动端 Get 式底部栏 + 聊一聊入口，批次 38） | 进行中（首批完成） |
| G1 | 链接采集后自动标签/摘要/抽取（复用既有队列） | 未开始 |
| G2 | AiSkill 技能注册表（点评/发芽/拷问/打磨）、作品层 WORKS、主动建议出口、个性化画像 | 未开始 |
| G3 | 主题知识库集合 + AI 归入建议、智能关联（建议制）、DIGEST 日报/周报、检索增强 | 未开始 |
